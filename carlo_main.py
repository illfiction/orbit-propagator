...
from monte_carlo.carlo_gs_analysis import monte_carlo_time_over_ground_station
...
def run_monte_carlo_simulation(config_path="config.json"):
    ...
    angular_velocities = np.loadtxt(
        "random_angular_velocities.csv", delimiter=",", skiprows=1
    )

    my_list = []

    print(
        f"Running simulation for {sim_params['time_span_days']} days with a timestep of {dt}s for the different values of angular velocities..."
    )
    ...
    for idx, initial_angular_velocity in enumerate(angular_velocities):
        ...
        for t in range(steps):
            ...
            if t % (steps // 10) == 0:
                print(
                    f"\rSimulation is {int(t / steps * 100)}% complete for the {idx + 1}th angular velocity...",
                    end="",
                    flush=True,
                )
        ...
        if analysis_params["run_ground_station_analysis"]:
            if ground_station_params:
                ground_station = GroundStation(
                    ground_station_params["name"],
                    ground_station_params["latitude_deg"],
                    ground_station_params["longitude_deg"],
                    ground_station_params["altitude_km"],
                    ground_station_params["min_elevation_deg"],
                )
                print(
                    f"Running ground station analysis for: {ground_station.name} for the {idx + 1}th angular velocity..."
                )

                my_tuple = monte_carlo_time_over_ground_station(
    ...
    print(
        "Here is the final list of average pointing intervals for different angular velocities:"
    )
    print(my_list)
